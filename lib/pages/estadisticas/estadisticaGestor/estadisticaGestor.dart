...
class EstadisticaGestorPage extends StatefulWidget {
  final int value;
  EstadisticaGestorPage({Key key, this.value}) : super(key: key);

  @override
  _EstadisticaGestorPageState createState() => _EstadisticaGestorPageState();
}

class _EstadisticaGestorPageState extends State<EstadisticaGestorPage> {
  ...
  _getClientes() async {
    ...
    // print(apiToken);
    final url =
        "$urlEstadisticasGestor/clientesTodos/${widget.value}?api_token="+apiToken;
    print(url);
    final response = await http.get(url);
    if (response.statusCode == 200) {
        final directory = await getApplicationDocumentsDirectory();
        final fileData = File('${directory.path}/pagEstadisticasEstadisticaGestorEstadisticaGestor${widget.value}.json');
