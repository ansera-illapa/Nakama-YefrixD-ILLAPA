...
class EstadisticaFMSectoresPage extends StatefulWidget {
  final int value;
  ...
  EstadisticaFMSectoresPage({Key key, 
                      this.value,
  ...
  @override
  _EstadisticaFMSectoresPageState createState() => _EstadisticaFMSectoresPageState();
}

class _EstadisticaFMSectoresPageState extends State<EstadisticaFMSectoresPage> {
  ...
  _getCliente() async {
    ...
    // print(apiToken);
    final url =
        "$urlEstadistica/filtroMayor/sectorista/${widget.value}?api_token="+apiToken;
    print(url);
    final response = await http.get(url);
    if (response.statusCode == 200) {
        final directory = await getApplicationDocumentsDirectory();
        final fileData = File('${directory.path}/pagEstadisticasEstadisticasSectoristasEstadisticasFMSectores${widget.value}.json');
