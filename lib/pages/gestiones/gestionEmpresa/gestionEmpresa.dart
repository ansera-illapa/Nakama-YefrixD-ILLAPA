...
class GestionEmpresaPage extends StatefulWidget {
  final int value;
  GestionEmpresaPage({Key key, this.value}) : super(key: key);

  @override
  _GestionEmpresaPageState createState() => _GestionEmpresaPageState();
}

class _GestionEmpresaPageState extends State<GestionEmpresaPage> {
  ...
  _getSocios() async {
    ...
    // print(apiToken);
    final url =
        "$urlGestion/clientesTodos/${widget.value}?api_token="+apiToken;
    print(url);
  ...
  _getTodosClientes() async{
    ...
    // print(apiToken);
    final url =
        "$urlGestion/clientesTodosExcepcion/${widget.value}?api_token="+apiToken;
    print(url);
    final response = await http.get(url);
    if (response.statusCode == 200) {
      final map = json.decode(response.body);
      final code = map["code"];
      final listClientes = map["result"];
      final sectoristaFreeSeleccionado = map["gestor"];
      final numeroDocumentos = map["numeroDocumentos"];
      final sumaImporteDocumentos = map["sumaImporteDocumentos"];
      final numeroDocumentosVencidos = map["numeroDocumentosVencidos"];
      final sumaImportesDocumentosVencidos = map["sumaImportesDocumentosVencidos"];
